...
*Note: As of 2025, I am nearly exclusively using [`uv`](https://docs.astral.sh/uv/) for dependency management. It is much faster and better at solving complicated environments. It has replaced both `conda`/`mamba` and `poetry` for me. I occasionally will use [`pixi`](https://pixi.sh/latest/) if I need to build a project that can only be compiled with `conda`-compatible distributions, as [it provides complete replacement functionality](https://pixi.sh/latest/switching_from/conda/).*
